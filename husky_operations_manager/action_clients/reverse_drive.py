...
class ReverseDriveClient:
    ...
    def __init__(self, node: Node, config: ReverseDriveConfig) -> None:
        ...
        # Timeout: accounts for speed ramp-down in last 0.5 m (multiplier 2.5)
        self._timeout = (abs(config.staging_x_offset) / self._lin_speed) * 2.5
    ...
    def drive_to_staging(self) -> bool:
        ...
        if self.config.dock_backwards:
            self.logger.error('dock_backwards=True — reverse drive unsafe. Manual intervention required.')
    ...
    def _control_loop(self) -> None:
        ...
        if self._start_time is None:
            ...
            self.logger.info(f'TF ready | start: ({robot_pose.pose.position.x:.3f}, {robot_pose.pose.position.y:.3f})')
    ...
    def _get_robot_pose(self, frame: str) -> PoseStamped | None:
        try:
            tf = self._tf_buffer.lookup_transform(frame, self.config.base_frame, Time(), timeout=Duration(seconds=0.2))
            pose = PoseStamped()
    ...
    def _compute_command(self, cmd: Twist, target: PoseStamped, robot_pose: PoseStamped) -> bool:
        ...
        # Scale linear speed down in the last 0.5 m to prevent overshoot
        speed_scale = min(1.0, dist / 0.5)
        linear_speed = self._lin_speed * max(speed_scale, 0.3)
        ...
        self.logger.debug(
            ...
            f'dist={dist:.3f}m | yaw_err={math.degrees(yaw_err):.1f}deg | '
            f'speed_scale={speed_scale:.2f} | cmd=(lin={cmd.linear.x:.3f}, ang={cmd.angular.z:.4f})',
            throttle_duration_sec=1.0,
        )
        return False
