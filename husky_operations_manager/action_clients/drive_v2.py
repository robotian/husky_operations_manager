...
class DriveConfig:
    ...
    No default values — every field must be explicitly supplied by the
    parent node to prevent silent misconfiguration.
    """

    # --- Subscriptions ---
    detection_topic: str    # ImageDetectionPose topic, relative to namespace
    odom_topic:      str    # Odometry topic, relative to namespace
    ...
    # --- Stop condition ---
    ex_tolerance:   float  # |ex| <= ex_tolerance + stop_lookahead → STOPPED (m)
    stop_lookahead: float  # Early stop margin to compensate detection drop coasting (m)
                           # Tune to: v_linear_min * expected_drop_gap_seconds

    # --- Coast gate ---
    ex_coast_gate: float   # Zero velocity on invalid detections within this range (m)
                           # Prevents coasting past stop point during detection drops

    # --- Angular gate ---
    ex_angular_gate: float  # |ex| <= ex_angular_gate → zero angular_z (m)
                             # Prevents yaw on final approach segment

    # --- Controller gain ---
    k_rho: float  # Proportional gain on camera Y centering error (heading error)
    ...
    no_detection_distance: float  # Distance without detection → row end assumed (m)
