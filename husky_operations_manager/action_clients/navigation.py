...
class NavigationActionClient:
    ...
    def __init__(self, node: Node) -> None:
        ...
        self.status_sub = self.node.create_subscription(
            RobotStatus, f'{self.namespace}/status/robot', self._robot_status_callback, 10
        )

        # Create action client
        self.client = ActionClient(self.node, NavigateThroughPoses, f'{self.namespace}/navigate_through_poses')
    ...
    def is_navigation_active(self) -> bool:
        ...
        """
        return self.navigation_status in [NavigationStatus.SENDING, NavigationStatus.ACCEPTED, NavigationStatus.ACTIVE]
    ...
    def send_goal(self, task: Task) -> bool:
        ...
        if self.is_navigation_active():
            self.logger.warning(
                f'Navigation already in progress (status: {self.navigation_status.name}) - skipping duplicate'
            )
            return False
        ...
        # Parse waypoints
        self.waypoints_list = [WayPoint(**wp) if isinstance(wp, dict) else wp for wp in self.sub_task.data]
        ...
        self.logger.info(
            f'Sending navigation goal: {self.waypoints_list[0].node_id} → {self.waypoints_list[-1].node_id}'
        )
    ...
    def _result_callback(self, future: Future) -> None:
        ...
        if status == 4:  # SUCCEEDED
            ...
            self.logger.warning(f'Navigation goal was aborted at waypoint {self._get_current_node_id()}')
    ...
    def _generate_goal_poses(self, waypoint_list: list[WayPoint]) -> list[PoseStamped]:
        """
            ...
            if index == 0:
                ...
                if next_wp:
                    ...
                    if abs(dx) > 0.01 or abs(dy) > 0.01:
                        ...
                        self.logger.debug(f'Waypoint {current_wp.node_id} → angle: {angle:.3f}')
    ...
    def _get_closest_waypoint_index(self, current_pose: Pose) -> int:
        ...
        current_dist = (current_wp.x - current_pose.position.x) ** 2 + (current_wp.y - current_pose.position.y) ** 2

        # Check all waypoints
        for i, waypoint in enumerate(self.waypoints_list):
            dist_squared = (waypoint.x - current_pose.position.x) ** 2 + (waypoint.y - current_pose.position.y) ** 2

            # Within threshold - consider reached
            if dist_squared <= threshold:
                if i != self.last_waypoint_index:
                    self.logger.debug(f'Reached waypoint {waypoint.node_id} (dist: {dist_squared**0.5:.3f}m)')
                return i
